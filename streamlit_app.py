...
# 데이터 로딩 및 초기화
df = pd.read_csv('merge_comment_0328.csv')
...
# 결과 표시
aggregated_html = aggregated_data.to_html(index=False)


# Streamlit에 HTML 코드를 가운데 정렬로 표시
st.markdown(f'<div style="display: flex; justify-content: center;">{aggregated_html}</div>', unsafe_allow_html=True)
st.markdown('<br>', unsafe_allow_html=True)

####################### Anova 분산 검정 ########################

if sentiment_option == 'all':
    # sentiment 별로 weighted_sentiment_score를 그룹화
    groups = filtered_df.groupby('sentiment')['weighted_sentiment_score'].apply(list)
    ...
    # P-value 해석
    if p_value < 0.05:
        st.write("ANOVA 분석 결과, 유의미한 차이가 있습니다.")
    else:
        st.write("ANOVA 분석 결과, 유의미한 차이가 없습니다.")

else:
    st.write("ANOVA 분석은 'all' 감성 옵션에서만 수행됩니다.")
...
def visualize_word_frequencies(words_df, top_n):
    ...
    top_words_df = words_df.head(top_n)
...
# LDA 모델링 실행
if st.toggle('LDA'):
    num_topics = st.number_input('토픽의 개수를 입력하세요', min_value=1, value=10)
    LDA_display(filtered_df['content'].tolist(), num_topics, 1200, 700)
